  ...
  useEffect(() => {
    return threads.unstable_on("run-end", async () => {
      ...
      if (!currentThread) {
        ...
        upsertThread(threadId, newThreadName, convertedMessages);
      } else {
        upsertThread(currentThread!.id, currentThread!.name, convertedMessages);
      }
    });
  }, [threads, currentThread, pathname]);
  ...
  return (
    <div className="mt-3 flex w-full items-stretch justify-center gap-4">
      <ThreadPrimitive.Suggestion
        className="cursor-pointer hover:bg-muted/80 flex max-w-sm grow basis-0 flex-col items-center justify-center rounded-lg border p-3 transition-colors ease-in"
        prompt="What are Cloudflare Workers?"
        method="replace"
        autoSend
      >
      ...
      <ThreadPrimitive.Suggestion
        className="cursor-pointer hover:bg-muted/80 flex max-w-sm grow basis-0 flex-col items-center justify-center rounded-lg border p-3 transition-colors ease-in"
        prompt="When are message queues helpful?"
        method="replace"
        autoSend
      >
        <span className="line-clamp-2 text-ellipsis text-sm font-semibold">
          When are message queues helpful?
        </span>
      </ThreadPrimitive.Suggestion>
    </div>
  );
